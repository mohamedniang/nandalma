...
export async function useUser(): Promise<IUser> {
  ...
  console.log('composable/useAuth#useUser@authCookie&user', authCookie, user.value)
...
export async function loginWithEmail(email: string, password: string) {
  const user = await $fetch('/api/auth/login', { method: 'POST', body: { email: email, password: password } }) as IUser
